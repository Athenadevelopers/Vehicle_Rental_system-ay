...
class FirebaseDBManager {
  ...
  async createCategory(categoryData) {
    try {
      ...
      return { success: true, category, id: categoryRef.id }
    } catch (error) {
  ...
  async getPopularVehicles(limitCount = 4) {
    try {
      if (!db) throw new Error("Firestore not initialized")

      // In a real app, you might want to sort by popularity metrics
  ...
  async addContactMessage(messageData) {
    try {
      ...
      const messageRef = doc(collection(db, this.collections.contactMessages))
      const message = {
        id: messageRef.id,
  ...
  async logActivity(userId, action, entityType = null, entityId = null, description = null) {
    try {
      ...
      const activity = {
        ...
        entityId: entityId,
        description: description,
        ipAddress: "unknown", // We can't reliably get IP on client side
